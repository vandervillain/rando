...
export const RoomContextProvider: FunctionComponent<RoomContextProps> = ({
  room,
}: RoomContextProps) => {
  console.debug(`<RoomContextProvider room=${room.id},${room.name} />`)
  ...
  const onPeerJoiningCall = useCallback(
    async (peer: RoomPeer) => {
      ...
      if (currUserPeer?.inCall) await requestStream(peer.id)
