...
namespace signalr_function.Functions
{
  public class RoomFunctions : ServerlessHub
  {
    ...
    {
      log.LogWarning($"{nameof(Login)}: {req.GetDisplayUrl()}");
    ...
    private async Task ExitRoom(string connId)
    {
      try
      {
        ...
        if (user != null && user.RoomId != null)
        {
          ...
          log.LogWarning($"told peers that {connId} exited room {user.RoomId}");
          await Groups.RemoveFromGroupAsync(connId, user.RoomId);
          log.LogWarning($"removed {connId} from group {user.RoomId}");
    ...
    {
      log.LogWarning($"{req.Method} {req.GetDisplayUrl()}");
    ...
    public void OnConnected([SignalRTrigger] InvocationContext context)
    {
      ...
      log.LogWarning($"{nameof(OnConnected)}: {context.UserId} connected as {context.ConnectionId}");
    }
    ...
    {
      roomMgr.ReconnectOrAddUser(context.UserId, context.ConnectionId);
      log.LogWarning($"{nameof(OnReconnected)}: {context.UserId} connected as {context.ConnectionId}");
    }

    [FunctionName(nameof(OnDisconnected))]
    public void OnDisconnected([SignalRTrigger] InvocationContext context)
    {
      log.LogWarning($"{nameof(OnDisconnected)}: {context.ConnectionId}");
    ...
    {
      log.LogWarning($"{nameof(SetUserName)}: {context.ConnectionId}");
    ...
    {
      log.LogWarning($"{nameof(CreateRoom)}: {context.ConnectionId}");
    ...
    {
      log.LogWarning($"{nameof(JoinRoom)}: {context.ConnectionId}");
    ...
    {
      log.LogWarning($"{nameof(JoinCall)}: {context.ConnectionId}");
    ...
    {
      log.LogWarning($"{nameof(LeaveRoom)}: {context.ConnectionId}");
      await ExitRoom(context.ConnectionId);
    }
    ...
    {
      log.LogWarning($"{nameof(Offer)}: {context.ConnectionId}");
      ...
      if (!string.IsNullOrWhiteSpace(userId) && offer != null)
      {
        ...
        {
          log.LogWarning($"user {user.Id} sending an offer to peer {peer.Id}");
    ...
    {
      log.LogWarning($"{nameof(Answer)}: {context.ConnectionId}");
      ...
      if (!string.IsNullOrWhiteSpace(userId) && answer != null)
      {
        ...
        {
          log.LogWarning($"user {user.Id} sending an answer to peer {peer.Id}");
    ...
    {
      log.LogWarning($"{nameof(Candidate)}: {context.ConnectionId}");
      ...
      if (!string.IsNullOrWhiteSpace(userId) && candidate != null)
      {
        ...
        {
          log.LogWarning($"user {user.Id} sending a candidate to peer {peer.Id}");
